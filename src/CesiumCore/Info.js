...
class PoupInfo {
  constructor(viewer, pick, element) {
    ...
    this._gisPosition = Cesium.Cartesian3.fromDegrees(
      pick.information.lon,
      pick.information.lat,
      0
    );
  }
  addPostRender() {
  ...
  poupEvent() {
    ...
    if (
      !(
        Cesium.Cartesian3.distance(camerPosition, this._gisPosition) > height
      ) &&
      this._viewer.camera.positionCartographic.height < 50000000
    ) {
