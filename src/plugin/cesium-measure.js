  ...
  Cesium.Measure = (function (Cesium) {
    ...
    function _(viewer, options = {}) {
      if (viewer && viewer instanceof Cesium.Viewer) {
        this._drawLayer = new Cesium.CustomDataSource(options.id); //当id为'measureLayer'的时候为测量模式
        this._drawLayer.flag = 0
        this._handlerDep = []
    ...
    _.prototype = {
      ...
      drawLineMeasureGraphics: function (options = {}) {
        if (this._viewer && options) {
          ...
          this._handlerDep.push(_handler)
          // left
          _handler.setInputAction(function (movement) {
            ...
            if (cartesian && cartesian.x) {
              ...
              // 添加量测信息点
              if (options.id == 'measureLayer') _addInfoPoint(cartesian);
      ...
      drawAreaMeasureGraphics: function (options = {}) {
        if (this._viewer && options) {
          ...
          _handler.setInputAction(function (movement) {
            ...
            // 添加信息点
            if (options.id == 'measureLayer') _addInfoPoint(positions[0]);
            if (typeof options.callback === "function") {
              options.callback(
                $this.transformCartesianArrayToWGS84Array(positions),
                polyObj
          ...
          function create(options) {
            ...
            let customLineMaterial = Cesium.Color.BLUE.withAlpha(0.8);
            let customAreaMaterial = Cesium.Color.BLUE.withAlpha(0.1);
            if (options.color >= 0) {
              customLineMaterial = Cesium.Color[colorArray[options.color]].withAlpha(0.8);
              customAreaMaterial = Cesium.Color[colorArray[options.color]].withAlpha(0.1);
            }
      ...
      cancelDraw() {
        if (this._handlerDep && this._handlerDep.length != 0) {
          this._handlerDep.forEach(item => {
