...
let myPieBox,
  ...
  detailChart,
  circleChart,
  scatterChart; //需要动态改变图表数据内容的要在最外层定义全局变量 然后需要在每次进入图表的时候调用销毁函数
          ...
          point: function (id, pointData) {
            ...
            let setSeries = function () {
              ...
              pointData.value.forEach((item, index) => {
                let seriesItem = {};
                var i = 0;
                (seriesItem.name = pointData.yAxis[index]),
                  (seriesItem.data = pointData.value[index]),
